...
def create_scheduled_emoji(lead_seconds: int = LEAD_SECONDS,
    ...
    recent_games = EmojiGame.objects.order_by("-starts_at")[:100]
    raw = pick_least_recently_used(
        pool=EMOJI_QUESTIONS,
        get_key_fn=lambda it: it.get("emoji"),
        recent_games=recent_games,
        get_game_keys_fn=lambda g: [q.get("emoji") for q in (g.questions or []) if isinstance(q, dict)],
        count=num_questions,
    )
