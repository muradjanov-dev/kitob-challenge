...
async def make_send_receipt_to_group_button(telegram_id: str, message_id: int):
    return InlineKeyboardMarkup(row_width=2).add(
        InlineKeyboardButton(
            _("✅ Ruxsat berish"), callback_data=f"accept:{telegram_id}:{message_id}"),
        InlineKeyboardButton(
            _("❗️ Rad etish"), callback_data=f"rejection:{telegram_id}:{message_id}"),
    )
